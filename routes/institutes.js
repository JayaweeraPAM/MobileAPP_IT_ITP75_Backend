...
institutesRouter.post('/:id/join', authMiddleware, async (req, res) => {
  try {
    ...
    const pendingRequest = allRequests.find(r => (r.tutorId === req.user.id || r.tutorId === req.user.userId) && String(r.status).toUpperCase() === 'PENDING');
    if (pendingRequest) {
      return res.status(400).json({ error: 'You already have a pending request.' });
    }

    const acceptedRequest = allRequests.find(r => (r.tutorId === req.user.id || r.tutorId === req.user.userId) && r.instituteId === institute.id && String(r.status).toUpperCase() === 'APPROVED');
    if (acceptedRequest) {
    ...
    if (t && t.instituteId && t.instituteId !== 'none') {
      if (t.instituteId === institute.id) {
        return res.status(400).json({ error: 'You are already a member of this institute.' });
      } else {
        return res.status(400).json({ error: 'You are already a member of another institute.' });
      }
    }
...
institutesRouter.get('/my/tutor-join-request', authMiddleware, async (req, res) => {
  try {
    ...
    const allRequests = await store.instituteJoinRequests.get() || [];
    const existing = allRequests.find(r => (r.tutorId === req.user.id || r.tutorId === req.user.userId) && String(r.status).toUpperCase() === 'PENDING');
...
institutesRouter.post('/join-requests/:requestId/approve', authMiddleware, async (req, res) => {
  try {
    ...
    if (t) {
      await store.tutors.updateOne(t.id || t.userId || reqItem.tutorId, { instituteId: institute.id });
    }
