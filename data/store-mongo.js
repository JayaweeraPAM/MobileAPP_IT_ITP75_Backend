...
export const store = {
  users: {
    ...
    getByEmail: async (email) => {
      ...
      // Prefer exact lowercase match, then fallback to case-insensitive lookup
      // so legacy mixed-case records can still log in.
      let doc = await db.collection('users').findOne({ email: normalizedEmail });
      if (!doc) {
